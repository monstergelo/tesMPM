...
{	
	s.initializeGrid(200, 100);
	s.initializeHelper();
	s.addParticles(102400);
	//createlookup();
...
void MPMFlui5App::draw()
{
	...
	gl::enableDepthWrite();
	mParticleBatch->draw();
	gl::drawString(toString(static_cast<int>(getAverageFps())) + " fps", vec2(632.0f, 52.0f));
	gl::drawString(toString(static_cast<int>(s.particles.size())) + " Partciles", vec2(632.0f, 75.0f));
}
